...
source "qemu" "autogenerated_1" {
  ...
  boot_command     = [
    ...
    "wget http://{{ .HTTPIP }}:{{ .HTTPPort }}/minikube<enter><wait>",
    "mv $PWD/minikube /mnt/etc/init.d/minikube<enter>",
    "chmod +x /mnt/etc/init.d/minikube<enter>",
    "umount /mnt<enter>",
    "reboot<enter>"
  ]
  boot_wait        = "60s"
...
build {
  ...
  provisioner "shell" {
    scripts = [
      ...
      "scripts/61-configure-minikube.sh",
